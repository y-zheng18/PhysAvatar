...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str, default='./output/sim_actorhq/')
    ...
    with torch.no_grad():
        src_v, src_f = lbs_deformer.read_obj(f'./data/{args.seq}/smplx_fitted/{str(args.start_frame).zfill(6)}.obj')

        lbs_w = np.load(f'./data/{args.seq}/optimized_weights.npy').astype(np.float32)

        smplx_param = torch.load(f'./data/{args.seq}/smplx_fitted/{str(args.start_frame).zfill(6)}.pth',
        ...
        t_human_v, transform_matrix, lbs_w = lbs_deformer.transform_to_t_pose(torch.from_numpy(fullbody_v).unsqueeze(0), smplx,
                                                                              smplx_param['trans'],
                                                                              smplx_param['scale'],
                                                                              lbs_w=torch.from_numpy(lbs_w).unsqueeze(0))
        pbar = tqdm.trange(args.frame_num + 1)
        pbar.set_description('Generating simulation input...')
        cloth_save_root = os.path.join('./sim_input', f'{args.seq}_{args.start_frame}_{args.frame_num}')
        fullbody_save_root = os.path.join('./sim_input', f'{args.seq}_{args.start_frame}_{args.frame_num}_full')
        smplx_save_root = os.path.join('./sim_input', f'{args.seq}_{args.start_frame}_{args.frame_num}_smplx')
        ...
        for i in pbar:
            smplx_param_i = torch.load(f'./data/{args.seq}/smplx_fitted/{str(i + args.start_frame).zfill(6)}.pth',
                                     map_location=torch.device('cpu') if not torch.cuda.is_available() else torch.device('cuda'))
            smplx_i = lbs_deformer.smplx_forward(smplx_param_i)
            human_v_i, transform_matrix_i = lbs_deformer.transform_to_pose(t_human_v, lbs_w, smplx_i,
                                                                           smplx_param_i['trans'],
                                                                           smplx_param_i['scale'])
            ...
            if i == 0:
                lbs_deformer.save_obj(os.path.join(cloth_save_root, 'dress_reorder.obj'), cloth_v_i[new_indices], cloth_f)
                ...
                lbs_deformer.save_obj(os.path.join(cloth_save_root, f'dress_reordered_{str(i)}.obj'), cloth_v_i, cloth_f)
        ...
        sim_args = {
            'algI': '1',
            'clothI': '2',
            'garmentName': f'{args.seq}_{args.start_frame}_{args.frame_num}',
            'seqName': f'{args.seq}_{args.start_frame}_{args.frame_num}_smplx',
        ...
        save_dir = os.path.join(args.save_dir, f'{args.seq}_{args.start_frame}_{args.frame_num}')
