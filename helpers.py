...
def project_3d_to_2d(points, K, RT):
...
def params2cpu(params, is_initial_timestep):
    if is_initial_timestep:
        ...
        res = {k: v.detach().cpu().contiguous().numpy() for k, v in params.items() if
               k in ['means3D', 'rgb_colors', 'unnorm_rotations', 'vertices', 'faces']}
    return res
...
def save_material(output_material, seq, exp):
    os.makedirs(f"./output/{exp}/{seq}", exist_ok=True)
    save_path = f"./output/{exp}/{seq}/material.pth"
    torch.save(output_material, save_path)