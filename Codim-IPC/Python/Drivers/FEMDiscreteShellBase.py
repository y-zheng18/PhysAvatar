...
class FEMDiscreteShellBase(SimulationBase):
    ...
    def set_DBC_index(self, last_index, v, rotCenter, rotAxis, angVelDeg):
        FEM.Init_Dirichlet_Index(self.X, self.DBC, last_index, self.DBCMotion, v, rotCenter, rotAxis, angVelDeg)
    ...
    def advance_one_time_step(self, dt):
        ...
        if self.t < self.MDBC_tmax and self.t > self.MDBC_tmin:
            if self.seqDBC < 0:
                ...
                path = self.garmentPath + '/garment_bd' + '/actorhq_smplx_' + str(self.curFrameNum) + '.txt'
                FEM.Step_Dirichlet_path(self.DBCMotion, dt, self.DBC, path)
                ...
                path = self.garmentPath + '/garment_bd' + '/actorhq_smplx_' + str(self.curFrameNum) + '.txt'
                FEM.Load_Dirichlet(self.seqDBCPath + '/shell' + str(self.curFrameNum) + '.obj', self.seqDBC, 1000000000, Vector3d(0, 0, 0), self.DBC)
                FEM.Step_Dirichlet_path(self.DBCMotion, dt, self.DBC, path)
                self.curFrameNum = self.curFrameNum + 1
