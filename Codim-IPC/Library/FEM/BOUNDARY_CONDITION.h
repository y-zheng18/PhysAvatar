...
//#####################################################################
template <class T, int dim>
void Init_Dirichlet_Index(MESH_NODE<T, dim>& X,
    ...
    const std::string& filePath)
{
    ...
    DBCMotion.Each([&](int id, auto data) {
        ...
        for (int i = range[0]; i < range[1]; ++i) {
            VECTOR<T, dim + 1>& dbcI = std::get<0>(DBC.Get_Unchecked(i));
            const VECTOR<T, dim>& coord = std::get<0>(X.Get_Unchecked(dbcI[0]));
            dbcI[1] = coord[0];
            dbcI[2] = coord[1];
            if constexpr (dim == 3) {
                dbcI[3] = coord[2];
    ...
    VECTOR_STORAGE<T, dim + 1>& DBC)
{
    ...
    DBC.Each([&](int id, auto data) {
        auto &[dbcI] = data;
        // if dbcI[0] - vIndOffset smaller than 0, it means the node index is not in the smplx mesh
        // not update the dbcI
        int vI = dbcI[0] - vIndOffset;
        if (vI >= 0 && dbcI[0] < vIndMax) {
            const VECTOR<T, dim>& coord = std::get<0>(X.Get_Unchecked(vI));
...
void Export_Boundary_Condition(py::module& m) {
    ...
    m.def("Init_Dirichlet_Index", &Init_Dirichlet_Index<double, 2>);
    m.def("Init_Dirichlet_Index", &Init_Dirichlet_Index<double, 3>);

    m.def("Step_Dirichlet", &Step_Dirichlet<double, 2>);
    m.def("Step_Dirichlet", &Step_Dirichlet<double, 3>);

    m.def("Step_Dirichlet_path", &Step_Dirichlet_path<double, 2>);
    m.def("Step_Dirichlet_path", &Step_Dirichlet_path<double, 3>);
