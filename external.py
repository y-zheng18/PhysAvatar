...
def densify(params, variables, optimizer, i):
    if i <= 5000:
        ...
        if (i >= 500) and (i % 100 == 0):
            ...
            if i >= 3000:
                remove_color_threshold = 0.05 if i == 5000 else 0.01    # remove points with low color
                to_remove_c = torch.logical_and(params['rgb_colors'][..., 0] < remove_color_threshold,
                                                params['rgb_colors'][..., 1] < remove_color_threshold)
                to_remove_c = torch.logical_and(params['rgb_colors'][..., 2] < remove_color_threshold, to_remove_c).squeeze()
                to_remove = torch.logical_or(to_remove, to_remove_c)
            if i >= 3000:
...
def mesh_cleaning(params, variables, optimizer, i):

    variables = accumulate_mean2d_gradient(variables)
    grad_thresh = 0.0002
    if (i >= 500) and (i % 100 == 0):
        # remove_threshold = 0.00
        # to_remove = (torch.sigmoid(params['logit_opacities']) < remove_threshold).squeeze()
        remove_color_threshold = 0.001  # remove points with low color
        to_remove = torch.logical_and(params['rgb_colors'][..., 0] < remove_color_threshold,
                                        params['rgb_colors'][..., 1] < remove_color_threshold)
        to_remove = torch.logical_and(params['rgb_colors'][..., 2] < remove_color_threshold, to_remove).squeeze()
        # to_remove = torch.logical_or(to_remove, to_remove_c)

        to_keep = torch.logical_not(to_remove)
        new_params = {k: v[to_keep] for k, v in params.items() if k not in ['cam_m', 'cam_c', 'vertices']}
        params = update_params_and_optimizer(new_params, params, optimizer)
        variables_ = {k: v[to_keep] for k, v in variables.items() if k not in ['scene_radius', 'face_neighbors']}
        variables.update(variables_)

        face_neighbors = find_adjacent_faces(variables['faces'].detach().cpu().numpy(), 3)
        variables['face_neighbors'] = torch.from_numpy(face_neighbors).cuda().long()
        ...
        # remove vertices with no faces
        vertices = params['vertices'].detach().cpu().numpy()
        faces = variables['faces'].detach().cpu().numpy()

        # Identify used vertices
        used_vertices = set(faces.flatten())

        # Map from old vertex indices to new ones
        new_vertex_indices = {old_idx: new_idx for new_idx, old_idx in enumerate(sorted(used_vertices))}

        # Update faces with new vertex indices
        updated_faces = np.array([[new_vertex_indices[vertex] for vertex in face] for face in faces])

        # Remove unused vertices
        updated_vertices = np.array([vertices[idx] for idx in sorted(used_vertices)])

        new_params = {'vertices': torch.from_numpy(updated_vertices).cuda().float(),}
        params = update_params_and_optimizer(new_params, params, optimizer)

        face_neighbors = find_adjacent_faces(updated_faces, 3)
        variables['face_neighbors'] = torch.from_numpy(face_neighbors).cuda().long()
        variables['faces'] = torch.from_numpy(updated_faces).cuda().long()
